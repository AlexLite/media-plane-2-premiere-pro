...
function assetTiles(): string {
  ...
  if (!assets.length) return `<div class="ff-empty-state"><div class="ff-empty-glyph">▦</div><p>${escape(activeSequence ? dt("sequenceNotExported") : dt("emptyAssets"))}</p>${activeSequence ? `<button class="ff-empty-action" data-direct-action="export">${escape(dt("exportSequence"))}</button>` : ""}</div>`;
  return `<div class="ff-asset-list">${assets.map(asset => { const version = asset.latest_version; const status = version?.processing_status ?? dt("missing"); return `<button class="ff-asset-tile${asset.id === selectedAsset ? " selected" : ""}" data-direct-asset="${escape(asset.id)}"><span class="ff-asset-thumb">${icon("play")}</span><span class="ff-asset-name">${escape(asset.name)}</span><span class="ff-asset-meta">${escape(version ? `v${version.version_number} · ${status}` : status)}</span></button>`; }).join("")}</div>`;
}
