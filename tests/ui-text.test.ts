...
describe("localized panel text", () => {
  it("does not embed visible alphabetic UI text directly in panel templates", () => {
    ...
    const directText = [...sources.matchAll(/<(?:h1|h2|h3|p|label|button|option|strong|span)[^>]*>\s*([\p{L}][^<]*)</gu)].map(match => match[1]);
    const textualAttributes = [...sources.matchAll(/<(?:input|select|button|textarea)[^>]*(?:placeholder|title|aria-label)="([^"]*[\p{L}][^"]*)"/gu)]
      .map(match => match[1])
