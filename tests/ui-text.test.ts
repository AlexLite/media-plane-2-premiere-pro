...
describe("localized panel text", () => {
  it("does not embed visible alphabetic UI text directly in panel templates", () => {
    const sources = ["main.ts", "review-panel.ts"].map(file => readFileSync(new URL(`../src/${file}`, import.meta.url), "utf8")).join("\n");
    const directText = [...sources.matchAll(/<(?:h1|h2|p|label|button|option|strong|span)[^>]*>\s*([A-Za-zА-Яа-я][^<]*)</g)].map(match => match[1]);
